...
use crate::wallet::{wallet_file, WalletState};
...
pub async fn resync_wallet() -> Result<()> {
    ...
    let config = crate::service::get_state::<Arc<Config>>();
    let wallet_id = config.get_wallet_id().await.into_tauri_result()?;

    let database_file = WalletState::wallet_database_path(&config, wallet_id)
        .await
        .into_tauri_result()?;
